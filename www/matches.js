...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadMatches() {
        try {
            ...
            if (cachedMatches && cacheTimestamp && (now - cacheTimestamp < 5 * 60 * 1000)) { // 5 minutes cache
                const data = JSON.parse(cachedMatches);
                logToNative('JS_LOG: Loaded matches from cache.');
                displayMatches(data);
                return;
            }

            const timestamp = new Date().getTime();
            const response = await fetch(`https://chaine-en-live.vercel.app/api/proxy?url=${encodeURIComponent(`https://raw.githubusercontent.com/amouradore/chaine-en-live/main/www/matches/today.json`)}&v=${timestamp}`, {
                cache: 'no-cache',
                headers: {
            ...
            logToNative('JS_LOG: Fetched match data from proxy.');
